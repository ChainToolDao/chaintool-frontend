			...
			meta: [
				{
					name: 'keyword',
					content:
						'以太币单位转换,Ethereum Unit Converter,gwei转eth,Wei转Gwei,Wei转换成ETH,Ether单位换算,加密货币转换器',
				},
			],
		}
	},
	...
	methods: {
		...
		convertWei(weiAmount) {
			this.ether_list.forEach((i) => {
				...
				if (i.type == 'Kwei') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 3).toString()
					)
				}

				if (i.type == 'Mwei') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 6).toString()
					)
				}

				if (i.type == 'Gwei') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 9).toString()
					)
				}

				if (i.type == 'Szabo') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 12).toString()
					)
				}

				if (i.type == 'Finney') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 15).toString()
					)
				}

				if (i.type == 'Ether') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 18).toString()
					)
				}

				if (i.type == 'Kether') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 21).toString()
					)
				}

				if (i.type == 'Mether') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 24).toString()
					)
				}

				if (i.type == 'Gether') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 27).toString()
					)
				}

				if (i.type == 'Tether') {
					i.value = this.deldempoint(
						ethers.utils.formatUnits(weiAmount, 30).toString()
					)
				}
			})
		},
